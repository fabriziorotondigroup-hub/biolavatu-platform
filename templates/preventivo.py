...
@login_required
def calcola_bp():
    """Calcola business plan in tempo reale"""
    data = request.json
    settings = Settings.query.first()

    macchine = data.get('macchine', [])
    scenario = data.get('scenario', 'realistico')
    mq = float(data.get('mq', 60))

    # Moltiplicatori scenario
    mult = {'pessimistico': 0.6, 'realistico': 1.0, 'ottimistico': 1.3}[scenario]
    ...
    for m in macchine:
        qty = int(m.get('qty', 1))
        prezzo = float(m.get('prezzo', 0))
        kw = float(m.get('kw', 0))
        tariffa = float(m.get('tariffa', 0))
        cicli = int(m.get('cicli_giorno', 0))
        comb = m.get('combustibile', 'elettrico')
        mc_ciclo = float(m.get('mc_ciclo', 0))

        capex += prezzo * qty
        kw_totale += kw * qty
        incasso += tariffa * cicli * qty * 26 * mult  # 26 giorni/mese
        cicli_giorno_tot += cicli * qty
        if comb == 'gas':
            gas_mc_giorno += mc_ciclo * cicli * qty
    ...
    costo_energia = kw_totale * 8 * 26 * kwh_cost  # 8h/giorno
    costo_gas = gas_mc_giorno * 26 * gas_cost
    # Acqua: ~50L/ciclo per lavatrice
    acqua_mc = cicli_giorno_tot * 0.05 * 26
    ...
    # Detergenti
    det1 = float(data.get('det1_costo_mese', 0) or 0)
    det2 = float(data.get('det2_costo_mese', 0) or 0)
    det3 = float(data.get('det3_costo_mese', 0) or 0)
    if not det1 and settings and cicli_giorno_tot > 0:
        det1 = (settings.det1_grammi_ciclo / 1000) * settings.det1_costo_kg * cicli_giorno_tot * 26
    if not det2 and settings and cicli_giorno_tot > 0:
        det2 = (settings.det2_grammi_ciclo / 1000) * settings.det2_costo_kg * cicli_giorno_tot * 26
    if not det3 and settings and cicli_giorno_tot > 0:
