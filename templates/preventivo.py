...
@login_required
def calcola_bp():
    """Calcola business plan — modello BIOLavaTU completo"""
    data = request.json
    settings = Settings.query.first()

    macchine = data.get('macchine', [])
    scenario = data.get('scenario', 'realistico')
    mult = {'pessimistico': 0.6, 'realistico': 1.0, 'ottimistico': 1.3}.get(scenario, 1.0)

    # ── 1. DATI ZONA ─────────────────────────────────────────────────────────
    pop_5min         = float(data.get('pop_5min', 0) or 0)
    pop_10min        = float(data.get('pop_10min', 0) or 0)
    concorrenti_500m = int(data.get('concorrenti_500m', 0) or 0)
    eta_media        = float(data.get('eta_media', 42) or 42)
    reddito_medio    = float(data.get('reddito_medio', 22000) or 22000)
    densita          = float(data.get('densita', 3000) or 3000)

    # ── 2. TASSO BASE per densità zona ───────────────────────────────────────
    if densita < 500:      tasso_base = 1/350.0   # industriale
    elif densita < 2000:   tasso_base = 1/180.0   # suburbana
    elif densita < 5000:   tasso_base = 1/120.0   # residenziale medio
    else:                  tasso_base = 1/70.0    # urbano denso

    # ── 3. MOLTIPLICATORI DEMOGRAFICI ────────────────────────────────────────
    if reddito_medio < 15000:    mult_reddito = 1.45
    elif reddito_medio < 20000:  mult_reddito = 1.20
    elif reddito_medio < 28000:  mult_reddito = 1.00
    elif reddito_medio < 38000:  mult_reddito = 0.80
    else:                        mult_reddito = 0.65

    if eta_media < 30:    mult_eta = 1.40
    elif eta_media < 38:  mult_eta = 1.20
    elif eta_media < 50:  mult_eta = 1.00
    elif eta_media < 60:  mult_eta = 0.85
    else:                 mult_eta = 0.70

    # ── 4. DOMANDA CICLI ─────────────────────────────────────────────────────
    bacino = pop_5min * 0.60 + pop_10min * 0.25
    cicli_std = bacino * tasso_base * mult_reddito * mult_eta
    # Extra piumoni/coperte/pet: 3% bacino lava 1 pezzo grande/settimana
    cicli_extra = bacino * 0.03 / 7.0
    quota = max(0.12, 1.0 - concorrenti_500m * 0.18)
    ...
    mult_access = 0.70 + (vis*0.15 + park*0.15 + piano*0.10 + h24*0.10)

    cicli_reali_giorno = (cicli_std + cicli_extra) * quota * mult_access * mult
    ...
    macchine_lav = [m for m in macchine if float(m.get('tariffa',0))>0
                    and 'asciug' not in m.get('nome','').lower()
                    and 'asciug' not in m.get('categoria','').lower()]
    macchine_asc = [m for m in macchine if float(m.get('tariffa',0))>0
                    and ('asciug' in m.get('nome','').lower()
                         or 'asciug' in m.get('categoria','').lower())]
    n_lav = sum(int(m.get('qty',1)) for m in macchine_lav)
    n_asc = sum(int(m.get('qty',1)) for m in macchine_asc)
    ...
    for m in macchine:
        ...
        if tariffa > 0:
            ...
            if is_asc:
                cicli_asc = (cicli_reali_giorno * 0.65 / n_asc) if n_asc > 0 else 0
                incasso_asciugatura += tariffa * cicli_asc * qty * 26
                cicli_giorno_tot    += cicli_asc * qty
                if comb == 'gas':
                    gas_mc_giorno += mc_ciclo * cicli_asc * qty
            else:
                cicli_lav = (cicli_reali_giorno / n_lav) if n_lav > 0 else 0
                incasso_lavaggio += tariffa * cicli_lav * qty * 26
                cicli_giorno_tot += cicli_lav * qty
                if comb == 'gas':
                    gas_mc_giorno += mc_ciclo * cicli_lav * qty
    ...
    # ── 8. AMMORTAMENTO E FINANZIAMENTO ──────────────────────────────────────
    ...
    capitale_fin  = capex * 1.22 * perc_fin
    if capitale_fin > 0 and anni_prest > 0:
        r = tasso_int / 12
        n = anni_prest * 12
        rata_mensile = capitale_fin * (r*(1+r)**n) / ((1+r)**n - 1)
    else:
        rata_mensile = 0
    ...
    costi_totali = (costo_energia + costo_gas + costo_acqua + costo_scarico +
                    ...
                    costo_lavoro + ammortamento + rata_mensile + det1 + det2 + det3)
    ...
    return jsonify({
        ...
        'cicli_giorno':        round(cicli_reali_giorno, 1),
        'cicli_piumoni':       round(cicli_extra * quota * mult_access * mult, 2),
        'dettaglio': {
            ...
            'finanziamento':  round(rata_mensile, 2),
