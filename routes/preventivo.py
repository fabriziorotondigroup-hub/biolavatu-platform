...
def genera_numero():
    ...
    # Usa MAX dell'id invece di COUNT — evita duplicati se pratiche vengono eliminate
    max_id = db.session.query(func.max(Pratica.id)).scalar() or 0
    # Verifica anche il numero più alto per l'anno corrente
    prefisso = f'BIO-{anno}-'
    ultima = db.session.query(Pratica.numero).filter(
        Pratica.numero.like(f'{prefisso}%')
...
def salva():
    ...
    # Calcola capex — usa prezzo_effettivo se presente, altrimenti prezzo
    capex = sum(
        float(m.get('prezzo_effettivo') or m.get('prezzo', 0)) * int(m.get('qty', 1))
        for m in macchine_sel
    )

    # Business plan — usa valori dal form se disponibili
    incasso  = float(data.get('incasso_mese', 0) or 0)
    costi    = float(data.get('costi_mese', 0) or 0)
    utile    = incasso - costi
    payback  = round((capex * 1.22) / utile / 12, 1) if utile > 0 else 0
