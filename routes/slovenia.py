"""
routes/slovenia.py — BIOLavaTU LaundryPro — Mercato Slovenia
...
from services.normativa_slovenia import NORMATIVA_SI
from services.ins_slovenia import (
    get_demographic_data_si, get_market_assessment_si, get_f_mesto_si,
    get_cambio_si_live, converti_eur_eur,
    get_densita_urbana_si,
    calcola_potenziale_lavaggi_si, calcola_incasso_da_lavaggi_si,
    calcola_affitto_max_si, calcola_costi_operativi_si, calcola_saturazione_si,
    TARIFFE_DEFAULT_SI, EUR_SIT_RATE, OCC_BASE_SI,
)
...
TR = {
    ...
    'pl': {
        'nuovo': 'Nova storitev', 'pratiche': 'Pratiche Slovenia',
...
def _get_lingua():
    return session.get('lingua_si', 'it')

si_bp = Blueprint('slovenia', __name__, url_prefix='/si')
...
def _check_pl():
    return current_user.role in ('owner','admin','segreteria') or \
           getattr(current_user, 'market', 'IT') == 'SI'

def _cambio():
    return get_cambio_si_live()
...
def dashboard():
    ...
    pratiche = Pratica.query.filter_by(market='SI').order_by(Pratica.created.desc()).limit(20).all()
    ling = _get_lingua()
    return render_template('polonia/dashboard_pl.html',
        pratiche=pratiche, cambio_si=_cambio(), lingua=ling, tr=TR.get(ling, TR['it']))

@si_bp.route('/pratiche')
@login_required
def pratiche():
    if not _check_pl(): return redirect(url_for('dashboard.index'))
    pratiche = Pratica.query.filter_by(market='SI').order_by(Pratica.created.desc()).all()
    return render_template('polonia/pratiche_pl.html', pratiche=pratiche, cambio_si=_cambio())

# ── Wizard ────────────────────────────────────────────────────────────────────
@si_bp.route('/preventivo/nuovo', methods=['GET'])
@login_required
def nuovo_preventivo():
    if not _check_pl(): return redirect(url_for('dashboard.index'))
    ling = _get_lingua()
    return render_template('polonia/preventivo_pl.html',
        pratica=None, cambio_si=_cambio(), tariffe=TARIFFE_DEFAULT_SI,
        settings=Settings.query.first(), lingua=ling, tr=TR.get(ling, TR['it']))

@si_bp.route('/preventivo/<int:id>')
@login_required
def modifica_preventivo(id):
    ...
    ling = _get_lingua()
    return render_template('polonia/preventivo_pl.html',
        pratica=p, cambio_si=_cambio(), tariffe=TARIFFE_DEFAULT_SI,
        settings=Settings.query.first(), lingua=ling, tr=TR.get(ling, TR['it']))

# ── API Geocodifica ───────────────────────────────────────────────────────────
@si_bp.route('/api/geocode')
@login_required
def geocode_pl():
    indirizzo = request.args.get('indirizzo','')
    citta     = request.args.get('citta','')
    si_reg      = request.args.get('si_reg','')
    if not GMAPS_KEY or not citta:
        return jsonify({'error': 'Parametri mancanti'}), 400
    addr = ', '.join(p for p in [indirizzo, citta, si_reg, 'Slovenia'] if p)
    try:
        r = requests.get('https://maps.googleapis.com/maps/api/geocode/json',
            params={'address': addr, 'key': GMAPS_KEY, 'region': 'si', 'language': 'pl'},
            timeout=8).json()
        ...
        demo = get_demographic_data_si(si_reg, citta)
        ass  = get_market_assessment_si(demo['reddito_medio'], demo['densita'])
...
def zona_analisi_pl():
    try:
        ...
        citta = request.args.get('citta','')
        si_reg  = request.args.get('provincia','')

        if (not lat or not lng) and citta and GMAPS_KEY:
            r = requests.get('https://maps.googleapis.com/maps/api/geocode/json',
                params={'address': citta+', Slovenia', 'key': GMAPS_KEY, 'region': 'pl'},
                timeout=5).json()
        ...
        demo = get_demographic_data_si(si_reg, citta)
        den_urbana = get_densita_urbana_si(si_reg, len(pois))
        pop3  = int(den_urbana * math.pi * 0.240**2)
        pop5  = int(den_urbana * math.pi * 0.400**2)
        pop10 = int(den_urbana * math.pi * 0.800**2)

        ass = get_market_assessment_si(demo['reddito_medio'], den_urbana)
        sc  = ass['score']
        ...
        if   sc >= 70: slabel, scol2 = 'Odlicno', '#10b981'
        ...
        return jsonify({
            ...
            'ins_data': {
                ...
                'reddito_medio':        demo.get('reddito_medio', 22000),
            },
            'paese': 'SI',
        })
    except Exception as e:
...
# ── API Business Plan ─────────────────────────────────────────────────────────
@si_bp.route('/api/calcola-bp', methods=['POST'])
@login_required
def calcola_bp_pl():
    d = request.json or {}
    cambio = _cambio()
    ...
    t_std = float(d.get('t_std') or TARIFFE_DEFAULT_SI['lavaggio_std_si'])
    t_med = float(d.get('t_med') or TARIFFE_DEFAULT_SI['lavaggio_med_si'])
    t_grd = float(d.get('t_grd') or TARIFFE_DEFAULT_SI['lavaggio_grd_si'])
    t_asc = float(d.get('t_asc') or TARIFFE_DEFAULT_SI['asciugatura_si'])
    c500  = int(d.get('concorrenti_500m',0)); c1k = int(d.get('concorrenti_1km',0))
    den   = float(d.get('densita', 4000)); red = float(d.get('reddito_medio', 22000))
    ...
    fc  = get_f_mesto_si(int(popc))
    msc = {'pessimistico':0.70,'realistico':1.00,'ottimistico':1.30}.get(sc_n, 1.0)
    occ = min(0.75, ob*(1+corr)*fc*msc)

    inc_si = ((n_std*18*t_std + n_med*18*t_med + n_grd*18*t_grd) + n_asc*52*t_asc)*occ*30
    aff_si = float(d.get('affitto_si', 900))
    cos_si = aff_si + inc_si*0.28 + 400
    uti_si = inc_si - cos_si
    cap_si = float(d.get('capex_si', 0))
    pb = (cap_si*1.23/uti_si/12) if uti_si > 0 else 999

    return jsonify({
        'incasso_si': round(inc_si), 'incasso_eur': round(get_cambio_si_live(inc_si)),
        'costi_si':   round(cos_si), 'utile_si':   round(uti_si),
        'utile_eur':   round(get_cambio_si_live(uti_si)),
        'capex_si':   round(cap_si), 'tva_si':     round(cap_si*0.23),
        'occupazione_pct': round(occ*100,1), 'f_miasto': round(fc,2),
        'cambio_si': cambio, 'payback_anni': round(pb,1) if pb<999 else None,
        'scenario': sc_n, 'valuta': 'EUR',
    })

# ── API Analisi AI ────────────────────────────────────────────────────────────
@si_bp.route('/api/analisi-ai', methods=['POST'])
@login_required
def analisi_ai_pl():
    import traceback
    try:
        ...
        cambio = _cambio()
        inc = float(d.get('incasso_si',0) or 0)
        cos = float(d.get('costi_si',  0) or 0)
        cap = float(d.get('capex_si',  0) or 0)
        red = float(d.get('reddito_medio',22000) or 22000)
        prompt = (
            "Sei un analista specializzato nel mercato delle lavanderie self-service in Slovenia. "
            "Produci un'analisi obiettiva della zona. Scrivi prima in ITALIANO, poi in SLOVENO (Slovenščina).\n\n"
            f"LOKALIZACJA: {d.get('indirizzo','')}, {d.get('citta','')}, Slovenia\n"
            f"Populacja 5 min: {int(d.get('pop_5min',0)):,} prebivalcev\n"
            f"Gestosc: {int(d.get('densita',0)):,} st/km2\n"
            f"Povprečna plača: {int(red):,} EUR/leto ({int(red/cambio):,} EUR/rok)\n"
            f"Konkurencja self-service 500m: {d.get('concorrenti_500m',0)}\n"
            f"Przychody szacowane: {inc:,.0f} EUR/mes\n"
            f"Koszty: {cos:,.0f} EUR/mes | Zysk: {inc-cos:,.0f} EUR/mes\n"
            f"Inwestycja: {cap:,.0f} PLN + DDV 22%\n\n"
            "Analisi in 4 sezioni (max 350 parole):\n"
            "1. BACINO DEMOGRAFICO / DEMOGRAFSKA OSNOVA\n"
            "2. CONCORRENZA / KONKURENCA\n"
            "3. PROIEZIONE ECONOMICA / EKONOMSKA PROJEKCIJA\n"
            "4. FATTORI DI RISCHIO / DEJAVNIKI TVEGANJA"
        )
        import anthropic
        client = anthropic.Anthropic(api_key=api_key)
        msg = client.messages.create(model='claude-sonnet-4-6', max_tokens=1200,
            messages=[{'role':'user','content':prompt}])
        return jsonify({'analisi': msg.content[0].text.strip(), 'mercato': 'SI'})
    except Exception as e:
        return jsonify({'errore': str(e), 'detail': traceback.format_exc()[-400:]}), 200

# ── API Cambio ────────────────────────────────────────────────────────────────
@si_bp.route('/api/cambio')
@login_required
def cambio_live():
    rate = _cambio()
    return jsonify({'eur_si': rate, 'si_eur': round(1/rate, 6)})

# ── API Demografici ───────────────────────────────────────────────────────────
@si_bp.route('/api/demografici')
@login_required
def demografici():
    si_reg   = request.args.get('si_reg','')
    miasto = request.args.get('miasto','')
    demo   = get_demographic_data_si(si_reg, miasto)
    ass    = get_market_assessment_si(demo['reddito_medio'], demo['densita'])
    return jsonify({**demo, **ass})

# ── Normativa ─────────────────────────────────────────────────────────────────
@si_bp.route('/normativa')
@si_bp.route('/normativa/<int:pratica_id>')
@login_required
def normativa(pratica_id=None):
    if not _check_pl(): return redirect(url_for('dashboard.index'))
    import datetime
    lingua = request.args.get('lingua', _get_lingua())
    norm   = NORMATIVA_SI.get(lingua, NORMATIVA_SI['it'])
    ...
    return render_template('polonia/normativa_pl.html',
        ...
        pratica_id=pratica_id, cambio_si=_cambio(),
...
@si_bp.route('/venditori')
@login_required
def venditori():
    if not _si_check(): return redirect(url_for('dashboard.index'))
    if not current_user.can_manage_venditori:
        flash('Accesso negato.', 'error')
        return redirect(url_for('slovenia.dashboard'))
    agenti = User.query.filter(
        User.market == 'SI',
        User.role.in_(('sales', 'sales_si', 'admin', 'segreteria'))
    ).order_by(User.created.desc()).all()
    ling = _get_lingua()
    return render_template('slovenia/venditori_si.html',
        agenti=agenti, lingua=ling, tr=TR.get(ling, TR['it']))


@si_bp.route('/venditori/nuovo', methods=['POST'])
@login_required
def nuovo_venditore_si():
    if not current_user.can_manage_venditori:
        return redirect(url_for('slovenia.dashboard'))
    email = request.form.get('email', '').strip()
    if not email or User.query.filter_by(email=email).first():
        flash('Email non valida o già registrata.', 'error')
        return redirect(url_for('slovenia.venditori'))
    u = User(
        nome   = request.form.get('nome', '').strip(),
        email  = email,
        role   = request.form.get('role', 'sales_si'),
        market = 'SI',
        attivo = True,
    )
    u.set_password(request.form.get('password', 'Slovenia2026!'))
    db.session.add(u)
    db.session.commit()
...
@si_bp.route('/venditori/<int:id>/toggle', methods=['POST'])
@login_required
def toggle_venditore_si(id):
    if not current_user.can_manage_venditori:
...
@si_bp.route('/venditori/<int:id>/elimina', methods=['POST'])
@login_required
def elimina_venditore_si(id):
...
# ── Lingua ────────────────────────────────────────────────────────────────────
@si_bp.route('/lingua/<lang>')
@login_required
def set_lingua(lang):
    if lang in ('it','si'): session['lingua_si'] = lang
    return redirect(request.referrer or url_for('slovenia.dashboard'))

# ── Salva pratica ─────────────────────────────────────────────────────────────
@si_bp.route('/preventivo/nuovo', methods=['POST'])
@login_required
def salva_nuovo():
    if not _check_pl(): return redirect(url_for('dashboard.index'))
    import datetime
    d = request.form; cambio = float(d.get('cambio_si') or _cambio())
    ...
    count  = Pratica.query.filter_by(market='SI').count() + 1
    numero = f"SI-{datetime.date.today().year}-{count:04d}"
    p = Pratica(
        numero=numero, cliente_id=c.id, agente_id=current_user.id,
        market='SI', stato='bozza',
        indirizzo=d.get('indirizzo',''), citta=d.get('citta',''),
        cap=d.get('cap',''), provincia=d.get('regija_cod',''),
        mq=int(d.get('mq') or 60),
        lat=float(d.get('lat') or 0) or None, lng=float(d.get('lng') or 0) or None,
        affitto_mese=float(d.get('affitto_si') or 0),
        ...
        capex=float(d.get('capex_si') or 0),
    ...
    return redirect(url_for('slovenia.modifica_preventivo', id=p.id))
